...
//        emit(IFEQ, thenLabel);
        emit(IFEQ, ctx.falseStatement() != null ? elseLabel : exitLabel);
//        emit(GOTO, ctx.falseStatement() != null ? elseLabel : exitLabel);
...
//        emitLabel(thenLabel);
    ...
    public void emitFor(PascalParser.ForStatementContext ctx)
    {
        ...
        char varType = ctx.variable().type == Predefined.integerType ? 'I' : 'C';

        compiler.visit(ctx.expression(0));
        emit(PUTSTATIC, programName + "/" + ctx.variable().getText() + " " + varType);

        emitLabel(loopLabel); // FOR statement loop label

        compiler.visit(ctx.variable());
        compiler.visit(ctx.expression(1));
        if (ctx.TO() != null) emit(IF_ICMPGT, loopExitLabel);
        else emit(IF_ICMPLT, loopExitLabel);

        compiler.visit(ctx.statement());

        compiler.visit(ctx.variable());
        // if its T0 load 1, if DOWNTO load -1
        if(ctx.TO() != null) emitLoadConstant(1);
        else emitLoadConstant(-1);
        // if TO adds 1 (increment), if DOWNTO add -1 (decrement)
        emit(IADD);
        emit(PUTSTATIC, programName + "/" + ctx.variable().getText() + " " + varType);
    ...
    public void emitProcedureCall(PascalParser.ProcedureCallStatementContext ctx)
    {
        ...
        if (ctx.argumentList() != null) {
            ArrayList<SymtabEntry> symtabEntryArrayList = compiler.programId.getRoutineSymtab().lookup(ctx.procedureName().getText()).getRoutineParameters();
            for(SymtabEntry e: symtabEntryArrayList){
                Typespec argType = e.getType();
                if (argType == Predefined.realType) procedureSignature += "F";
                else if (argType == Predefined.charType) procedureSignature += "C";
                else if (argType == Predefined.integerType) procedureSignature += "I";
                else procedureSignature += "Z";      // boolean
            }
            procedureSignature += ")V";
        }
        else{
            procedureSignature = "()V";
        }
        emit(INVOKESTATIC, programName +  "/" + ctx.procedureName().getText() + procedureSignature);
    }
    ...
    public void emitFunctionCall(PascalParser.FunctionCallContext ctx)
    {
        ...
        String procedureSignature = "(";
        if (ctx.argumentList() != null) {
            ArrayList<SymtabEntry> symtabEntryArrayList = compiler.programId.getRoutineSymtab().lookup(ctx.functionName().getText()).getRoutineParameters();
            for(SymtabEntry e: symtabEntryArrayList){
                Typespec argType = e.getType();
                if (argType == Predefined.realType) procedureSignature += "F";
                else if (argType == Predefined.charType) procedureSignature += "C";
        ...
        //TODO: functions may have an explicit return type
        emit(INVOKESTATIC, programName +  "/" + ctx.functionName().getText() + procedureSignature);
    }
